...
def test_iso_sorting_blanks_and_other_columns(registered, display):
    with patch.object(callbacks, "_get_enriched_register_display_df", return_value=(display, "old count")):
        records, _, count = registered[0](*FILTERS, 20, [2023, 2024], 2023, 2024)
...
def test_inclusive_narrowing_widening_and_reset(registered, display, reset):
    with patch.object(callbacks, "_get_enriched_register_display_df", return_value=(display, "old count")):
        lower, _, count = registered[0](*FILTERS, 20, [2023, 2023], 2023, 2024)
        upper, _, singular = registered[0](*FILTERS, 20, [2024, 2024], 2023, 2024)
        restored, _, _ = registered[0](*FILTERS, 20, reset, 2023, 2024)
        empty, _, empty_count = registered[0](*FILTERS, 20, [2022, 2022], 2023, 2024)
...
def test_global_bounds_and_complete_native_pages(registered, display):
    ...
    with patch.object(callbacks, "_get_enriched_register_display_df", return_value=(subset, "old count")) as getter:
        records, page, count = registered[0](*filters, 10, [2023, 2023], 2019, 2026)
        getter.assert_called_once_with(*filters)
        restored, _, _ = registered[0](*filters, 10, [2019, 2026], 2019, 2026)
...
def test_download_range_filters_and_presentation(registered, display, years, expected):
    ...
    with (
        ...
        result = registered[1](1, *filters, years, 2023, 2024)
        getter.assert_called_once_with(*filters)
...
def test_real_search_composes_with_year_and_download(registered):
    ...
    records, _, count = registered[0](*filters, 10, [year, year], *bounds)
    ...
    assert records == expected.to_dict("records")
    assert count == f"Showing {len(selected):,} accreditation record{'s' if len(selected) != 1 else ''}"
    downloaded = registered[1](1, *filters, [year, year], *bounds)
    assert downloaded["content"] == selected.to_csv(index=False)
    restored, _, _ = registered[0](*FILTERS, 10, None, *bounds)
    assert len(restored) == len(full) > 10


def test_layout_settings_global_bounds_help_and_registration(registered, display):
    actual, _ = callbacks._get_enriched_register_display_df(*FILTERS)
    for source in (actual, display):
        with patch.object(layout, "_get_enriched_register_display_df", return_value=(source, "old count")) as getter:
            tab = layout.build_thematic_tab()
            getter.assert_called_once_with(*FILTERS)
        dates = pd.to_datetime(source["Accreditation Date"], format="%d %b %Y", errors="coerce")
        ...
        assert ("Records without a usable accreditation date" in text) == dates.isna().any()
        props = _find_component(tab, "enriched-register-table").to_plotly_json()["props"]
        assert props["sort_action"] == "native" and props["filter_action"] == "none"
