...
re-running keeps existing codes for unchanged rule IDs and appends new ones.
"""
...
def entries():
    ...
    # A label such as Unclear from Register Entry exists in two layers, so its
    # rules are named by layer; otherwise two rules would share one name.
    duplicated = {c["label"] for c in live if sum(1 for x in live if x["label"] == c["label"]) > 1}
    rows = []
    for layer in LAYER_ORDER:
        for c in sorted((x for x in live if x.get("layer") == layer), key=lambda x: x["label"]):
            label = c["label"]; kind = kind_of.get(layer, layer)
            shown = f"{label} ({kind})" if label in duplicated else label
    ...
    for key in PRINCIPLES:
        if key in meta:
            rows.append({"scope": "principle", "category": "", "kind": "principle", "rule_type": key,
                         "name": "Principle: " + key.replace("_", " "), "excerpt": flatten(meta[key])})
    for key, value in (meta.get("cross_layer_assignment_principles") or {}).items():
        rows.append({"scope": "principle", "category": "", "kind": "principle", "rule_type": key,
                     "name": "Principle: " + key.replace("_", " "), "excerpt": flatten(value)})
    return rows
...
def main():
    ...
    for row in rows:
        if row["name"] in existing:
            out.append(existing[row["name"]]); continue
