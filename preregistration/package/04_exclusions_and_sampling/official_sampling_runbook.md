...
Use this runbook only after excluded pilot completion and debrief, closure of the dated pilot-feedback log, Jo's final substantive review, propagation of resulting changes, final package QA, Gate 1 freeze, official preregistration verification, and Gate 2 authorisation. The canonical manifest must record one current implementation basis with `frozen`, `registered`, and `official_sample_draw_authorised` all true. Under review candidate v0.14 they remain false, so this runbook is not executable. The package does not contain a registration receipt or sampled identities.
