...
STATUS_VISIBILITY_QUESTION = (
    "Could this status reasonably have been determined from the public title and datasets "
    "above?"
)
LLM_DISCLAIMER = (
    "The proposed classifications below were generated by a large language model and are "
    "being evaluated. They are not presented as definitive descriptions of the project."
)
...
QUESTIONNAIRE_FIELD_LABELS = {
    **{
        f"po_d{slot:02d}_vis": CLASSIFICATION_VISIBILITY_QUESTION
    ...
    **{
        f"po_p{slot:02d}_vis": CLASSIFICATION_VISIBILITY_QUESTION
    ...
    **{
    ...
    "po_t01_vis": STATUS_VISIBILITY_QUESTION,
    "po_t01_vis_explain": (
    ...
    "po_t02_vis": STATUS_VISIBILITY_QUESTION,
    "po_t02_vis_explain": (
        "Please briefly explain why the basis for this proposed tag status is only partly "
        "visible, not visible, or unclear in the public project title and listed datasets."
    ),
...
def normalise_authority_text(value: str) -> str:
    """Normalise authority text for provenance checks without changing output text."""
...
def check_sources() -> None:
    ...
    consent_by_field = {
        str(item["dictionary_field"]): normalise_authority_text(
            str(item["statement_text"])
        )
        for item in consent_statements
    }
    if tuple(consent_by_field) != CONSENT_NAMES:
        raise RuntimeError("canonical consent statement field set or order changed")
    if consent_by_field["consent_no_nonpublic"] != dict(CONSENT_ITEMS)[
        "consent_no_nonpublic"
    ]:
        raise RuntimeError("approved consent_no_nonpublic statement differs")
...
def build_dictionary() -> tuple[list[dict[str, str]], dict[str, object]]:
    ...
    by_name["participant_info_link"]["Field Label"] = (
        INLINE_PARTICIPANT_INFO_SOURCE.read_text(encoding="utf-8")
    )
    ...
    old_owner_guidance = (
        "Please do not forward this personalised link and do not enter confidential, sensitive "
        "or otherwise non-public information. Each project review normally takes approximately "
        "3–5 minutes."
    )
    new_owner_guidance = (
    ...
    for row in rows:
        if (
            "@READONLY-SURVEY" in row["Field Annotation"]
            or "@HIDDEN-SURVEY" in row["Field Annotation"]
        ):
            row["Required Field?"] = ""

    by_name["po_privacy"]["Field Label"] = (
        f"<strong>Important</strong><br>{FORM_GUIDANCE_WORDING}"
    )
...
def patch_generated_specs(rows: list[dict[str, str]]) -> None:
    ...
    for row in field_rows:
        ...
        if name == "participant_info_link":
            ...
            row["construct"] = "participant_facing_llm_status_disclaimer"
            row["notes"] = (
                "Exact Questionnaire v3 wording immediately before po_intro and the proposed-"
                "classification judgements."
            )
        elif name in CONSENT_NAMES:
    ...
    branch["owner_consent_v3"] = {
        ...
        "inline_participant_information_source": str(
            INLINE_PARTICIPANT_INFO_SOURCE.relative_to(ROOT)
        ),
        "inline_participant_information_sha256": INLINE_PARTICIPANT_INFO_SHA256,
        "inline_participant_information_field": "participant_info_link",
    ...
    branch["project_review_v3"] = {
        ...
        "llm_disclaimer_field": "po_llm_disclaimer",
        "llm_disclaimer_wording": LLM_DISCLAIMER,
...
def patch_formatting_audit() -> None:
    ...
    for row in rows:
        if row["variable_name"] == "participant_info_link":
            ...
            row["remaining_live_qa_requirement"] = (
                "Verify the full inline information renders legibly above the ten consent "
                "confirmations on desktop and mobile."
            )
    ...
    rows.append(
        {
            "variable_name": "po_llm_disclaimer",
            "instrument": "project_review",
            "participant_visible_purpose": "Per-review large-language-model status disclaimer",
            "contains_heading": "no",
            "heading_text": "",
            "body_text": LLM_DISCLAIMER,
            "html_tags_used": "div",
            "whole_block_bold_present_before_correction": "no",
            "final_formatting_status": "normal-weight descriptive text",
            "remaining_live_qa_requirement": (
                "Verify placement after project information and before classification guidance."
            ),
        }
    )
    rows.append(
