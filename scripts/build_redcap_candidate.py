...
def build_dictionary():
 ...
 field('sc_domains','scratch_coder','checkbox','Research Domain(s)','Classification',choice(dl),'Select every supported substantive domain, or Unclear alone.',required=True,annotation="@NONEOFTHEABOVE='12'"),field('sc_purposes','scratch_coder','checkbox','Analytical Purpose(s)',choices=choice(pl),note='Select one or at most two purposes, or Unclear alone.',required=True,annotation="@MAXCHOICE=2 @NONEOFTHEABOVE='8'"),field('sc_covid','scratch_coder','radio','COVID-19 & Pandemic',choices='0, No | 1, Yes',required=True),field('sc_equity','scratch_coder','radio','Demographic disparities / equity tag',choices='0, No | 1, Yes',required=True),
 field('sc_sufficiency','scratch_coder','radio','Register-entry sufficiency','Evidence and confidence','1, Sufficient | 2, Partial | 3, Insufficient',required=True),field('sc_taxonomy_fit','scratch_coder','radio','Taxonomy fit',choices='1, Fit | 2, Partial Fit | 3, No Fit',required=True),field('sc_tax_issue','scratch_coder','checkbox','Taxonomy issue type',choices='1, Missing category | 2, Ambiguous/overlapping categories | 3, Too broad | 4, Too narrow | 5, Other | 6, None',branch="[sc_taxonomy_fit] = '2' or [sc_taxonomy_fit] = '3'",required=True),field('sc_confidence','scratch_coder','radio','Classification confidence',choices='1, High | 2, Medium | 3, Low',required=True),field('sc_note','scratch_coder','notes','Explanatory note',note='Required for exposure, Partial/Insufficient evidence, Partial/No Fit, or Low confidence.',branch="[sc_exposure] = '1' or [sc_sufficiency] = '2' or [sc_sufficiency] = '3' or [sc_taxonomy_fit] = '2' or [sc_taxonomy_fit] = '3' or [sc_confidence] = '3'",required=True)]
...
def build_specs(rows,meta):
 ...
 spec={'version':VERSION,'forms':['assignment_admin','scratch_coder','project_owner'],'scratch':{'domain_field':'sc_domains','domain_unclear_code':12,'domain_unclear_exclusive':True,'purpose_field':'sc_purposes','purpose_unclear_code':8,'purpose_min':1,'purpose_max':2,'purpose_unclear_exclusive':True,'required_core':['sc_blind_decl','sc_exposure','sc_domains','sc_purposes','sc_covid','sc_equity','sc_sufficiency','sc_taxonomy_fit','sc_confidence'],'conditional_required':{'sc_exposure_note':'sc_exposure == 1','sc_tax_issue':'sc_taxonomy_fit in [2,3]','sc_note':'sc_exposure == 1 or sc_sufficiency in [2,3] or sc_taxonomy_fit in [2,3] or sc_confidence == 3'},'action_tags_for_live_confirmation':{'sc_domains':"@NONEOFTHEABOVE='12'",'sc_purposes':"@MAXCHOICE=2 @NONEOFTHEABOVE='8'"}},'owner':{'label_mapping':meta['owner_mapping'],'fit_codes':{1:'Fits',2:'Does not fit',3:'Unsure'},'visibility_codes':{1:'Clearly visible',2:'Partly visible',3:'Not visible',4:'Unsure'},'missing_branches':{'po_miss_domains':'po_miss_domain == 1','po_miss_purposes':'po_miss_purpose == 1','po_miss_tags':'po_miss_tag == 1'},'conditional_note':'any proposed fit != 1 or visibility != 1; any missing-label report; sufficiency != 1; taxonomy_fit != 1','contradiction_rule':'Proposed-and-Fits plus missing for the same label is rejected.'},'neutral_assignment_id':{'pattern':'^[A-Z0-9]{8}$','forbidden_tokens':['baseline','hard','reserve','active','coder','owner'],'must_not_derive_from_hidden_id':True},'completion':{'generated_fields':['assignment_admin_complete','scratch_coder_complete','project_owner_complete'],'invalid_if_trigger_unresolved':True}}
...
def build_preview(rows):
 ...
 admin=''.join(f"<li>{html.escape(r['Variable / Field Name'])}: {html.escape(r['Field Label'])}</li>" for r in rows if r['Form Name']=='assignment_admin')
 doc=f"<!doctype html><html lang=en><head><meta charset=utf-8><title>REDCap candidate preview</title><style>body{{font-family:Arial;max-width:960px;margin:2rem auto}}section{{border-top:1px solid #ccc}}.warning{{background:#fff3cd;padding:1rem}}</style></head><body><h1>REDCap candidate instrument preview</h1><p class=warning>Synthetic structural preview only; not a pixel-perfect REDCap rendering. Confirm appearance and action tags in UCL REDCap before the pilot.</p>{''.join(forms)}<h2>Administrative appendix (hidden from reviewers)</h2><ul>{admin}</ul></body></html>"
 (PACKAGE/'redcap_candidate_instrument_preview.html').write_text(doc,encoding='utf-8')

def build_fixtures():
 sc={'assignment_id':'A7K3M9Q2','sc_blind_decl':1,'sc_exposure':0,'sc_domains':[1],'sc_purposes':[1],'sc_covid':0,'sc_equity':0,'sc_sufficiency':1,'sc_taxonomy_fit':1,'sc_confidence':1}; po={'assignment_id':'Q4N8Z2L7','cluster_id':'SYNTH-PROJECT-A','owner_resp_id':'SYNTH-OWNER-A','prop_d01':1,'po_d01_fit':1,'po_d01_vis':1,'po_miss_domain':0,'po_miss_purpose':0,'po_miss_tag':0,'po_sufficiency':1,'po_taxonomy_fit':1}; cases=[]
 def add(cid,stream,valid,base,**kw): d=dict(base); d.update(kw); cases.append({'case_id':cid,'stream':stream,'expected_valid':valid,'data':d})
 add('sc_01_ordinary','scratch',True,sc); add('sc_02_multi_domain','scratch',True,sc,sc_domains=[1,2]); add('sc_03_two_purpose','scratch',True,sc,sc_purposes=[1,2]); add('sc_04_unclear_domain','scratch',True,sc,sc_domains=[12]); add('sc_05_unclear_conflict','scratch',False,sc,sc_domains=[1,12]); add('sc_06_three_purpose','scratch',False,sc,sc_purposes=[1,2,3]); add('sc_07_partial_note','scratch',True,sc,sc_sufficiency=2,sc_note='Public entry gives only partial evidence.'); add('sc_08_insufficient_note','scratch',True,sc,sc_sufficiency=3,sc_note='The analytical operation is not visible.'); add('sc_09_low_confidence','scratch',True,sc,sc_confidence=3,sc_note='A rule boundary remains uncertain.'); add('sc_10_taxonomy_issue','scratch',True,sc,sc_taxonomy_fit=2,sc_tax_issue=[2],sc_note='Two categories overlap.'); add('sc_11_exposure','scratch',True,sc,sc_exposure=1,sc_exposure_note='Unexpected non-permitted context was visible.',sc_note='Exposure was reported.'); inc=dict(sc); inc.pop('sc_purposes'); add('sc_12_incomplete_core','scratch',False,inc)
 add('po_13_all_fit','owner',True,po); add('po_14_not_fit','owner',True,po,po_d01_fit=2,po_note='The proposed domain does not fit.'); add('po_15_unsure','owner',True,po,po_d01_fit=3,po_note='I am unsure.'); add('po_16_basis_not_visible','owner',True,po,po_d01_vis=3,po_note='The basis is not public.'); add('po_17_missing_domain','owner',True,po,po_miss_domain=1,po_miss_domains=[2],po_note='Education is missing.'); add('po_18_missing_purpose','owner',True,po,po_miss_purpose=1,po_miss_purposes=[2],po_note='Outcome tracking is missing.'); add('po_19_missing_tag','owner',True,po,po_miss_tag=1,po_miss_tags=[1],po_note='The equity tag is missing.'); add('po_20_taxonomy_concern','owner',True,po,po_taxonomy_fit=2,po_tax_issue=[1],po_note='A category is missing.'); add('po_21_owner_a','owner',True,po); add('po_21_owner_b','owner',True,dict(po,assignment_id='R5T9X3C8',owner_resp_id='SYNTH-OWNER-B')); add('po_22_contradiction','owner',False,po,po_miss_domain=1,po_miss_domains=[1],po_note='Contradictory synthetic case.')
 FIXTURES.mkdir(parents=True,exist_ok=True); (FIXTURES/'redcap_candidate_synthetic_submissions.yaml').write_text(yaml.safe_dump({'fixture_status':'synthetic_only','contains_real_record_ids':False,'cases':cases},sort_keys=False),encoding='utf-8')
